...
<title>travelTrackStatusPage</title>
         	...
         	<table class="table table-striped">
         		<tr>
         			...
         			<th><span style="font-weight: bolder; text-align: right; font-size: 18px">
         				<c:set var="travel" value="${travelStatus.status }"/>
				         	<c:set var="status" value="Rejected (by FSO)"/>
				         	
				         	<c:if test="${travel==status}">
				         		<c:out value="REMARKS"/>
				         	</c:if>
         			</span></th>
         			...
         			<tr>
						...
						<td><span style="font-weight: bolder; text-align: right; font-size: 16px">${travelStatus.status}</span></td>
						<td><span style="font-weight: bolder; text-align: right; font-size: 16px">
							<c:set var="travel" value="${travelStatus.status }"/>
				         	<c:set var="status" value="Rejected (by FSO)"/>
				         	<c:set var="remarks" value="${travelStatus.remarks}"/>
				         	
				         	<c:if test="${travel==status}">
				         		<c:out value="${travelStatus.remarks}"/>
				         	</c:if>
						</span></td>
         			</tr>
